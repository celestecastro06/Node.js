...
pokemon.get('/:id([0-9]{1,3})', async (req, res, next) => {
   ...
   if (id >= 1 && id <= 722) {
	const pkmn = await db.query("SELECT * FROM pokemon WHERE pok_id="+id+";");
     res.status(200).json({code: 200, message: pokemon});
   }
   return res.status(404).send({code: 200, message:"Pokemon no encontrado."});
});

pokemon.get('/:name([A-Za-z]+)', async (req, res, next) => {
	const name = req.params.name;
	const pkmn = await db.query("SELESCT * FROM pokemon WHERE pok_name='"+name+"';");
	if (pkmn.length > 0) {
		res.status(200).json({code: 200, message: pokemon});
	}
	return res.status(404).send({code: 200, message:"Pokemon no encontrado."});
